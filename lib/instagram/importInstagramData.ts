// lib/instagram/importInstagramData.ts
//
// Shared import logic — used by both:
//   • app/api/import/instagram/route.ts  (HTTP API route)
//   • app/api/instagramPipeline/instagramPipeline.ts  (CLI / cron script)
...
export interface InstagramClaimsData {
  instagram: {
    inputUrl: string
    id: string
    username: string
...
export interface ImportInstagramResult {
  success: boolean
  summary: {
    sourceCreated: boolean
    documentCreated: boolean
    socialMediaUpdated: boolean
    factsCreated: number
    postFactsCreated: number
    postsUpserted: number
    evidenceLinked: number
    clinicWebsiteUpdated: boolean
  }
  error?: string
}
...
export async function importInstagramData(
  ...
  let clinicWebsiteUpdated = false
  if (instagramData.instagram.externalUrls?.length) {
    ...
    if (existingClinic && !existingClinic.website_url) {
      await supabase
        .from('clinics').update({ website_url: websiteUrl }).eq('id', clinicId)
      clinicWebsiteUpdated = true
    }
  }
