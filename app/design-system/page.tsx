...
const NAV_SECTIONS: NavItem[] = [
  ...
  { id: "layout", label: "Container & Section" },
...
export default function DesignSystemPage() {
  ...
  return (
    <div className="imConnectTheme min-h-screen bg-background">
      <div className="flex gap-8 p-6 lg:p-8 max-w-[1600px] mx-auto">
              ...
              <div key={item.id}>
                ...
                {item.children?.map((child) => (
                  <a
                    key={child.id}
                    href={`#${child.id}`}
                    className="block rounded-md pl-6 pr-3 py-1.5 text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-colors"
                  >
                    {child.label}
                  </a>
                ))}
        ...
        <main className="flex-1 min-w-0 space-y-12">
          ...
          <Card id="atomic-design" className="scroll-mt-24">
            <CardHeader>
              <CardTitle>Atomic Design Methodology</CardTitle>
              <CardDescription>
                Components are organized by abstraction level per{" "}
                <a
            ...
            <CardContent className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div id="atoms" className="rounded-lg border bg-card p-4 scroll-mt-24">
                  <h3 className="font-semibold mb-2">Atoms</h3>
                  <p className="text-sm text-muted-foreground mb-3">
                    Foundational building blocks: Button, Input, Label, Badge, Checkbox, Slider, Separator, Heading, Text, Icons.
                  </p>
                  <p className="text-xs text-muted-foreground">Location: <code className="bg-muted px-1 rounded">components/ui/</code>, <code className="bg-muted px-1 rounded">components/icons/</code></p>
                </div>
                <div id="molecules" className="rounded-lg border bg-card p-4 scroll-mt-24">
                  <h3 className="font-semibold mb-2">Molecules</h3>
                  <p className="text-sm text-muted-foreground mb-3">
                    Simple groups of atoms: SpecialtyTag, StatBlock, FilterPillGroup, RangeValueDisplay, PageTitle, PageSubtitle, PriceRatingBlock, FeeLineItem, VerificationBadge, IconActionLink, Card.
                  </p>
                  <p className="text-xs text-muted-foreground">Location: <code className="bg-muted px-1 rounded">components/ui/</code></p>
                </div>
                <div id="organisms" className="rounded-lg border bg-card p-4 scroll-mt-24">
                  <h3 className="font-semibold mb-2">Organisms</h3>
                  <p className="text-sm text-muted-foreground mb-3">
                    Complex sections: ClinicCard, HeroBanner, FeatureCard, FilterDialog, SummarySidebar, TopNav, profile sections.
                  </p>
                  <p className="text-xs text-muted-foreground">Location: <code className="bg-muted px-1 rounded">components/landing/</code>, <code className="bg-muted px-1 rounded">components/istanbulmedic-connect/</code></p>
                </div>
                <div id="templates" className="rounded-lg border bg-card p-4 scroll-mt-24">
                  <h3 className="font-semibold mb-2">Templates</h3>
                  <p className="text-sm text-muted-foreground mb-3">
                    Page-level layout structure: HomeTemplate defines slots for hero, matching, testimonials, journey, partners.
                  </p>
                  <p className="text-xs text-muted-foreground">Location: <code className="bg-muted px-1 rounded">components/templates/</code></p>
                </div>
              </div>
              <p className="text-sm text-muted-foreground">
                Pages (e.g., <code className="bg-muted px-1 rounded">app/page.tsx</code>) are instances of templates with real content. See{" "}
                <code className="bg-muted px-1 rounded">components/ui/README.md</code> for the full taxonomy.
              </p>
            </CardContent>
          </Card>
        ...
        <Card id="badge" className="scroll-mt-24">
          ...
          <CardContent className="space-y-6">
            ...
            <div>
              ...
              <p className="im-text-body-xs im-text-muted mb-4">Component: <code className="bg-muted px-2 py-1 rounded">SpecialtyTag</code></p>
              <div className="flex flex-wrap gap-2 items-center">
                {["Hair Transplant", "Dental", "Cosmetic Surgery", "Eye Surgery", "Bariatric Surgery", "Plastic Surgery"].map(
                  (label, index) => (
                    <SpecialtyTag
                      key={label}
                      label={label}
                      variant={TAG_VARIANT_SEQUENCE[index % TAG_VARIANT_SEQUENCE.length]}
                    />
                  )
                )}
              </div>
        ...
        {/* Dialog Section */}
        <Card id="dialog" className="scroll-mt-24">
          <CardHeader>
            <CardTitle>Dialog Component</CardTitle>
            <CardDescription>Modal overlay with DialogContent, DialogHeader, DialogFooter, DialogTrigger, DialogClose. Used in FilterDialog, HeroSection lightbox, ReviewsSection.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <p className="im-text-body-sm im-text-muted mb-4">Component: <code className="bg-muted px-2 py-1 rounded">Dialog</code></p>
            <div>
              <h3 className="im-heading-4 mb-4">Basic Dialog with Trigger</h3>
              <Dialog>
                ...
                <DialogContent className="sm:max-w-[425px]">
                  <DialogHeader>
                    ...
                    <DialogDescription>Dialog description text goes here. This modal is used for filters, lightboxes, and review modals.</DialogDescription>
                  </DialogHeader>
                  <div className="py-4">
                    <p className="im-text-body-sm">Dialog content area. Use DialogClose for custom close buttons in the header.</p>
                  </div>
                  <DialogFooter>
                    <Button variant="outline">Cancel</Button>
                    <Button variant="teal-primary">Confirm</Button>
                  </DialogFooter>
                </DialogContent>
              </Dialog>
            </div>
            <div className="im-text-body-xs im-text-muted space-y-1">
              <p>Exports: Dialog, DialogContent, DialogHeader, DialogFooter, DialogTitle, DialogDescription, DialogTrigger, DialogClose</p>
              <p>Used in: FilterDialog (filters modal), HeroSection (image lightbox), ReviewsSection (all reviews modal)</p>
            </div>
          </CardContent>
        </Card>

        {/* Label Section */}
        <Card id="label" className="scroll-mt-24">
          <CardHeader>
            <CardTitle>Label Component</CardTitle>
            <CardDescription>Form labels for Checkbox and Input association. Uses Radix Label primitives.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <p className="im-text-body-sm im-text-muted mb-4">Component: <code className="bg-muted px-2 py-1 rounded">Label</code></p>
            <div>
              <h3 className="im-heading-4 mb-4">Label with Checkbox</h3>
              <div className="space-y-4">
                <div className="flex items-center space-x-2">
                  <Checkbox id="label-check1" />
                  <Label htmlFor="label-check1" className="cursor-pointer">Accept terms and conditions</Label>
                </div>
                <div className="flex items-center space-x-2">
                  <Checkbox id="label-check2" />
                  <Label htmlFor="label-check2" className="cursor-pointer text-base font-normal">JCI Accredited</Label>
                </div>
              </div>
            </div>
            <div>
              <h3 className="im-heading-4 mb-4">Label with Input</h3>
              <div className="space-y-2">
                <Label htmlFor="label-email">Email address</Label>
                <Input id="label-email" placeholder="you@example.com" />
              </div>
            </div>
            <p className="im-text-body-xs im-text-muted">Used in: FilterDialog for treatment, accreditation, and language filter labels.</p>
          </CardContent>
        </Card>

        {/* ScrollArea Section */}
        <Card id="scroll-area" className="scroll-mt-24">
          <CardHeader>
            <CardTitle>ScrollArea Component</CardTitle>
            <CardDescription>Scrollable region with custom scrollbar. Used in FilterDialog for scrollable filter content.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <p className="im-text-body-sm im-text-muted mb-4">Component: <code className="bg-muted px-2 py-1 rounded">ScrollArea</code></p>
            <div>
              <h3 className="im-heading-4 mb-4">Scrollable Content</h3>
              <ScrollArea className="h-[200px] w-full rounded-md border p-4">
                <p className="im-text-body mb-2">ScrollArea provides a custom-styled scrollbar for overflow content.</p>
                <p className="im-text-body mb-2">Use when content may exceed a fixed height (e.g. filter dialogs, long lists).</p>
                <p className="im-text-body mb-2">Exports: ScrollArea, ScrollBar (for horizontal/vertical orientation).</p>
                <p className="im-text-body mb-2">The FilterDialog uses ScrollArea to contain treatment, budget, language, and accreditation filters.</p>
                <p className="im-text-body mb-2">Placeholder to demonstrate scroll: Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
              </ScrollArea>
            </div>
          </CardContent>
        </Card>
        ...
        <Card id="layout" className="scroll-mt-24">
          <CardHeader>
            <CardTitle>Layout: Container & Section</CardTitle>
            <CardDescription>Core layout primitives. Container provides max-width and responsive padding; Section wraps page sections.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-8">
            <div>
              <h3 className="im-heading-4 mb-4">Container</h3>
              <p className="im-text-body-sm im-text-muted mb-4">Component: <code className="bg-muted px-2 py-1 rounded">Container</code></p>
              <p className="im-text-body-sm mb-2">max-w-screen-2xl mx-auto px-3 sm:px-6 lg:px-8</p>
              <p className="im-text-body-xs im-text-muted mb-4">Used in TopNav, Header, HeroBanner, and all landing sections.</p>
              <div className="border rounded-lg overflow-hidden">
                <Container className="bg-muted/50 py-4">
                  <p className="im-text-body text-center">Content inside Container</p>
                </Container>
              </div>
            </div>
            <div>
              <h3 className="im-heading-4 mb-4">Section</h3>
              <p className="im-text-body-sm im-text-muted mb-4">Component: <code className="bg-muted px-2 py-1 rounded">Section</code></p>
              <p className="im-text-body-sm mb-2">Props: id, className, noPadding (removes default padding for hero-style sections)</p>
              <p className="im-text-body-xs im-text-muted mb-4">Used in HeroBanner, PrecisionClinicMatching, RealPeopleSection, PersonalMedicalConcierge, JourneyToConfidence, EliteMedicalPartners.</p>
              <div className="border rounded-lg overflow-hidden">
                <Section id="demo-section" className="py-6 px-4 bg-muted/30">
                  <p className="im-text-body text-center">Content inside Section</p>
                </Section>
              </div>
            </div>
          </CardContent>
        ...
        {/* Carousel Section */}
        <Card id="carousel" className="scroll-mt-24">
          <CardHeader>
            <CardTitle>Carousel Component</CardTitle>
            <CardDescription>Embla-based carousel. Carousel, CarouselContent, CarouselItem. Used in RealPeopleSection for transformation slides.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <p className="im-text-body-sm im-text-muted mb-4">Component: <code className="bg-muted px-2 py-1 rounded">Carousel</code></p>
            <div>
              <h3 className="im-heading-4 mb-4">Basic Carousel</h3>
              <Carousel className="w-full max-w-md mx-auto">
                <CarouselContent>
                  <CarouselItem>
                    <div className="p-6 border rounded-lg bg-muted/30 text-center">Slide 1</div>
                  </CarouselItem>
                  <CarouselItem>
                    <div className="p-6 border rounded-lg bg-muted/30 text-center">Slide 2</div>
                  </CarouselItem>
                  <CarouselItem>
                    <div className="p-6 border rounded-lg bg-muted/30 text-center">Slide 3</div>
                  </CarouselItem>
                </CarouselContent>
              </Carousel>
            </div>
            <p className="im-text-body-xs im-text-muted">Exports: Carousel, CarouselContent, CarouselItem, CarouselApi. RealPeopleSection uses setApi for auto-advance and navigation.</p>
          </CardContent>
        </Card>

        {/* Motion & Animation Section */}
        <Card id="motion" className="scroll-mt-24">
          <CardHeader>
            <CardTitle>Motion & Animation</CardTitle>
            <CardDescription>FadeInUp, FadeIn, ScaleIn. Viewport-triggered or immediate. Used in PageTitle, PageSubtitle, and landing sections.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-8">
            <p className="im-text-body-sm im-text-muted mb-6">Components: <code className="bg-muted px-2 py-1 rounded">FadeInUp</code>, <code className="bg-muted px-2 py-1 rounded">FadeIn</code>, <code className="bg-muted px-2 py-1 rounded">PageTitle</code>, <code className="bg-muted px-2 py-1 rounded">PageSubtitle</code></p>
            <div>
              <h3 className="im-heading-4 mb-4">FadeInUp (reveal on scroll)</h3>
              <FadeInUp className="p-4 border rounded-lg bg-muted/30">
                <p className="im-text-body">This block animates in when it enters the viewport. Uses IntersectionObserver with amount 0.25, once: true by default.</p>
              </FadeInUp>
            </div>
            <div>
              <h3 className="im-heading-4 mb-4">FadeIn (immediate, no viewport)</h3>
              <FadeIn reveal={false} className="p-4 border rounded-lg bg-muted/30">
                <p className="im-text-body">Animates immediately on mount. Use for hero/above-the-fold content with reveal=false.</p>
              </FadeIn>
            </div>
            <div>
              <h3 className="im-heading-4 mb-4">PageTitle & PageSubtitle</h3>
              <div className="space-y-2 p-4 border rounded-lg bg-muted/30">
                <PageTitle as="h2" reveal={false}>Section Title</PageTitle>
                <PageSubtitle reveal={false}>Subtitle using typography and FadeInUp. Used across landing sections.</PageSubtitle>
              </div>
            </div>
            <div className="im-text-body-xs im-text-muted space-y-1">
              <p>Props: reveal (default true), amountOverride (viewport threshold), once (default true)</p>
              <p>Used in: PageTitle, PageSubtitle, HeroBanner, PrecisionClinicMatching, PersonalMedicalConcierge</p>
            </div>
          </CardContent>
        </Card>
